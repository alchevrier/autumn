...
- **Strictly Unidirectional Temperature-Bounded I/O:** Eliminates dual-channel confusion and circular locking bugs. Ingress into the CPU is natively treated as **Hot** (`@BoundaryChannel`), polling tightly in the L1 cache. Egress is treated as **Cold** (`@ColdChannel`), ensuring fire-and-forget I/O pipelines that physically segregate external network/disk backpressure away from computational hot-paths.
...
Using `.mmap()` natively, the Hot Path (e.g. Market Data parsing running on isolated cores 4-15) writes lock-free indices directly to memory via explicitly unidirectional `@ColdChannel` fire-and-forget handoffs. Downstream Cold Path applications (Risk, DB Logging, Websockets on shared cores 0-3) map the same memory block as their ingress `@BoundaryChannel` and read sequentially.
This physically eliminates **backpressure** and bidirectional circular dependencies! A slow database write on Core 0 can never lock or stall the Hot Strategy execution on Core 4. You achieve the sub-microsecond determinism of a monolith, but the restartability/crash-tolerance of distributed microservices.
