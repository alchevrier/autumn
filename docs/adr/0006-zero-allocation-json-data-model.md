...
- This model applies specifically to arbitrarily nested or dynamic data objects that cross a network boundary and must be forwarded exactly as received: state documents, configuration documents, auth payloads, and credential envelopes.
- **Exception: high-throughput structured data (lists and forms) uses an Array of Structs (AoS) layout.** Data that is rendered in high quantities (paginated list rows, ADR-0011) or highly mutable but structurally fixed (form state slots, ADR-0012), as well as read-only configuration tables, are stored in flat pre-allocated AoS pools. Each entry occupies a fixed-size slot at a known byte offset in a pre-allocated `ByteArray` (a Simple Binary Encoding or SBE view). Access is an index block multiplied by an offset read; no JSON navigation is needed at runtime. The pool size and slot layout are derived from the configuration schema. For this data, the ingress parse cost (decoding JSON to AoS) is paid once at the network boundary, ensuring ultra-fast O(1) reads during scrolling or typing.

### Concrete AoS/SBE mechanism
...
**Why not chosen for all documents:**
- Autumn's core state documents arrive as JSON from remote APIs and are often forwarded as JSON to remote APIs. A purely pool-and-SBE model for *everything* requires a decode step (JSON → pool) on ingress and an encode step (pool → JSON) on egress. These are one-time boundary costs per document, but they reintroduce allocation and parsing overhead at the network boundary that the raw JSON view avoids entirely.
- By using the hybrid approach, Autumn applies SBE where structurally dense data requires O(1) performance (lists, forms, config tables), and strictly avoids parsing overhead for opaque or dynamic documents crossing the network.
...
- The pool approach becomes the clear winner for all layers if Autumn's wire format ever moves to binary end-to-end. If the API contract changes from JSON to SBE or FlatBuffers natively, the one-time ingress/egress cost disappears and the pool model is strictly superior everywhere. This decision should be revisited at that point.
