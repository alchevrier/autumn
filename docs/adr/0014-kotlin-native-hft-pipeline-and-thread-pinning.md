...
2. **State-Level Thread Pinning & Thread Allocation Budgets:** Because Kotlin Native interops directly with POSIX (`#include <pthread.h>`), we endorse explicit CPU core affinity (`pthread_setaffinity_np`) per FSM State. 
   ...
   - **Scheduler IRQ Eviction (OS Partitioning):** Because specific cores are statically claimed via `@ExecutionNode(pinnedCore = N)`, the compiler can auto-generate the required Linux kernel boot parameters (`isolcpus`, `nohz_full`, `rcu_nocbs`, `irqaffinity`). This literally disables the OS scheduler tick and hardware interrupts on the pinned FSM cores. The OS is completely evicted from the core, leaving 100% of the CPU cycles dedicated to the Autumn poll loop with absolute zero jitter.
...
7. **IDE Tooling and Microarchitectural Analysis (IntelliJ Plugin):** Because these boundaries and budgets are strictly declared via static annotations, an IntelliJ IDEA plugin can parse the AST in real-time. This provides engineers with a visual "Hardware Synthesis" view directly in their IDE:
   - **Wire Maps:** Highlighting ring-buffer connections between FSM component blocks.
