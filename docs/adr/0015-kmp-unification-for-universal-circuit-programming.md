...
1. **Kotlin/Native (LLVM) for Linux HFT (Bare-Metal):**
   - **Mechanism:** Compiles to raw Linux machine code (`linuxX64`). The compiler automatically maps `@NetworkChannel` to true kernel-bypass (DPDK/`AF_XDP`), generating `/dev/shm` IPC Shared Memory for `@SharedChannel`, alongside `pthread` core pinning and GRUB `isolcpus` hooks for absolute zero-jitter execution.
