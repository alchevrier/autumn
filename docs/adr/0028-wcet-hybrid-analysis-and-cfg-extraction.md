...
5. **Topology JSON Serialisation is Procedural:** Currently, `TopologyExportSerializer` writes nodes to `topology.json` individually as the K2 compiler encounters them during Pass 1. If an `@InjectTopology` pipeline root is scanned *before* its `@BoundaryChannel` dependencies, the root fails to serialize correctly. To accurately reflect the true `auditable=true` structural graph, the compiler must perform a multi-pass accumulation step, linking all isolated channels to their topological roots in memory, and only dumping to JSON during the final generation closure phase.
6. **Multi-Pass Hardware Scaling Verification:** The static CFG analysis currently limits cycles for a *single pass* through the execution nodes. However, empirical physical execution via `perf` will count the *entire lifetime* of the running native footprint (e.g. running 1,000,000 benchmark polling boundaries on execution entry). True certificaiton requires a mechanism correlating the exact number of FSM iterations to intelligently scale the aggregate compiler cycle budget before comparing it to empirical physical CPU lifecycle results.
7. **FSM State Activity Weighting:** Simple loop multiplication assumes every channel fires simultaneously and every branch resolves to its worst-case path recursively. True WCET requires the mathematical solver to incorporate Probabilistic Convolution or Active Rates (e.g., normal pathways execute 99% of the time, error channels execute 1%, and only 2 of 5 bounds can structurally be hot on the same physical cycle).
   * **Future Application (Machine Learning & Auto-Correction):** The explicit physical topology telemetry lays the absolute foundation for dynamic model weighting. A background ML observation loop could continually sample live FSM branch histograms and empirical tracing offsets in production, feeding these runtime-adjusted path probabilities back into the static ILP solver to automatically tighten WCET budgets in CI/CD without human intervention.
