...
class TopologySynthesisTransformer(
    ...
    override fun visitFunctionNew(declaration: IrFunction): IrStatement {
        ...
        messageCollector.report(CompilerMessageSeverity.INFO, "[Autumn Topology] Detected @InjectTopology hook: ${declaration.name.asString()}")
                ...
                if (handler != null) {
                    ...
                    if (propertyType != null) {
                        ...
                        if (pollFunc != null && commitPollFunc != null) {
                            if (isNetwork) {
                                messageCollector.report(CompilerMessageSeverity.INFO, "[Autumn Topology] Synthesizing burst routing loop for BoundaryChannel '${channel.name.asString()}' -> handler: '$targetHandlerName'")
                                
                                val chanVal = irTemporary(irCall(propertyGetter))
                                
                                // Generate: while(true) { val idx = chanVal.poll(); if (idx == -1) break; handler(idx); chanVal.commitPoll() }
                                val loopType = pluginContext.irBuiltIns.unitType
                                val whileLoop = IrWhileLoopImpl(-1, -1, loopType, org.jetbrains.kotlin.ir.expressions.IrStatementOrigin.WHILE_LOOP)
                                ...
                                messageCollector.report(CompilerMessageSeverity.INFO, "[Autumn Topology] Injecting standard processing loop for '${channel.name.asString()}'")
                                val chanVal = irTemporary(irCall(propertyGetter))
